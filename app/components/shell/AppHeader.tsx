...
import { Bell, Search, Settings, ChevronDown, Bot, FileText, User, Loader2, Menu, X, LayoutGrid, Network } from 'lucide-react'
...
const MOBILE_NAV = [
  ...
  { label: 'Knowledge units', href: '/knowledge-units', icon: Network },
...
export function AppHeader({
  ...
  return (
    <>
    <div className="sticky top-0 z-40 flex h-16 items-center gap-3 border-b border-[var(--color-border)] bg-[var(--color-bg-primary)] px-4 md:gap-6 md:px-6">
      <button
        type="button"
        onClick={() => setMobileNavOpen(true)}
        aria-label="Abrir menú"
        className="flex h-10 w-10 items-center justify-center rounded-[var(--radius-md)] text-[var(--color-text-secondary)] hover:bg-[var(--color-hover)] md:hidden"
      >
        <Menu size={20} />
      </button>
      <div className="flex items-center md:w-[216px]">
        <Logo size={28} />
      </div>
    ...
    {mobileNavOpen && (
      <div className="fixed inset-0 z-50 md:hidden">
        ...
        />
        <aside className="absolute left-0 top-0 flex h-full w-72 max-w-[85vw] flex-col bg-[var(--color-bg-primary)] shadow-2xl">
          <div className="flex items-center justify-between border-b border-[var(--color-border)] px-4 py-3">
            ...
            <button
              ...
              className="flex h-9 w-9 items-center justify-center rounded-[var(--radius-md)] text-[var(--color-text-secondary)] hover:bg-[var(--color-hover)]"
            >
              <X size={18} />
            </button>
          </div>
          <nav className="flex flex-1 flex-col gap-1 overflow-y-auto p-3">
            {MOBILE_NAV.map(({ label, href, icon: Icon }) => (
              <Link
                ...
                href={href}
                onClick={() => setMobileNavOpen(false)}
                className="flex items-center gap-3 rounded-[var(--radius-md)] px-3 py-3 text-sm font-medium text-[var(--color-text-primary)] no-underline hover:bg-[var(--color-bg-secondary)]"
              >
                <Icon size={18} />
                {label}
              </Link>
          ...
          <div className="border-t border-[var(--color-border)] p-3">
            <div className="flex items-center gap-3 px-2 py-2">
              ...
              <div className="min-w-0 flex-1">
                <div className="truncate text-sm font-medium text-[var(--color-text-primary)]">{userName}</div>
                <div className="truncate text-xs text-[var(--color-text-tertiary)]">{userEmail}</div>
              </div>
            </div>
          </div>
        </aside>
