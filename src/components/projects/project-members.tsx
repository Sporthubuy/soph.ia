    ...
    <section className="panel p-6 space-y-5">
      ...
      <ul className="space-y-2">
        {members.map((m) => {
          ...
          return (
            ...
            >
              <div className="w-9 h-9 rounded-full bg-[#e1e0ff] text-[#4648d4] flex items-center justify-center label-sm font-semibold flex-shrink-0">
                {initials(m.profiles?.full_name ?? null, mail)}
              </div>

              <div className="flex-1 min-w-0">
                <p className="body-md text-black font-medium truncate">
                  {name}
                  {isOwner && (
                    <span className="ml-2 label-sm text-[#7c839b] font-normal">
                      Responsable
                    </span>
                  )}
                </p>
                {mail && name !== mail && (
                  <p className="body-sm text-[#7c839b] truncate">{mail}</p>
                )}
              </div>
              ...
              {canManage && !isOwner ? (
                <div className="flex items-center gap-2">
                  <select
                    ...
                    className="body-sm border border-[#e2e8f0] rounded-lg px-2 py-1.5 bg-white text-[#45464d] focus:outline-none focus:ring-2 focus:ring-[#4648d4]"
                  >
                    {ROLES.map((r) => (
                ...
                <span className="label-sm px-2 py-1 rounded border border-[#e2e8f0] text-[#45464d]">
                  {roleLabel(m.role)}
                </span>
              )}
            </li>
      ...
      {invitations.length > 0 && (
        <div className="space-y-2">
          <p className="label-sm text-[#7c839b]">PENDIENTES DE REGISTRARSE</p>
          <ul className="space-y-2">
            {invitations.map((inv) => (
              <li
                key={inv.id}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="body-md text-[#45464d] truncate">{inv.email}</p>
      ...
      {canManage && (
        <form onSubmit={handleInvite} className="space-y-2 pt-1">
          <label htmlFor="invite-email" className="label-sm text-[#7c839b]">
            INVITAR POR EMAIL
          </label>
          <div className="flex gap-2 flex-wrap">
            <input
              ...
              className="flex-1 min-w-[14rem] px-4 py-2.5 border border-[#e2e8f0] rounded-lg bg-white text-black placeholder-[#7c839b] focus:outline-none focus:ring-2 focus:ring-[#4648d4] focus:border-transparent body-sm"
            />
            <select
              value={role}
              onChange={(e) => setRole(e.target.value)}
              aria-label="Permiso del invitado"
              className="body-sm border border-[#e2e8f0] rounded-lg px-3 py-2.5 bg-white text-[#45464d] focus:outline-none focus:ring-2 focus:ring-[#4648d4]"
            >
              {ROLES.map((r) => (
                <option key={r.value} value={r.value}>
                  {r.label}
            ...
            <button
              ...
              className="bg-[#4648d4] text-white font-medium py-2.5 px-4 rounded-lg hover:bg-[#3a3ab0] transition-colors disabled:opacity-50 body-sm"
            >
      ...
      {notice && <p className="body-sm text-green-700">{notice}</p>}
    </section>
  );
};
